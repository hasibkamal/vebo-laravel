...
use App\Api\Transformers\SalesCompanyTransformer;
...
class SalesCompanyApiController extends ApiController
{
    ...
    public function lastCompanyId(Request $request)
    {
        try{
            ...
            $prefix = 'SC';
            $companyId = DB::select("SELECT CONCAT('$prefix',LPAD(IFNULL(MAX(SUBSTR(table2.company_id,-5,5) )+1,1),5,'0')) AS company_id FROM (SELECT * FROM sales_companies ) AS table2 WHERE table2.company_id LIKE '$prefix%'")[0]->company_id;
